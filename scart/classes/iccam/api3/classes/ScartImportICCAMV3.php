...
class ScartImportICCAMV3 extends ScartGenericICCAMV3 {
    ...
    /**
     * @descripton find unactioned and unassessed content items in ICCAM and process them
     *
     */
    public function do()
    {
        ...
        try {
            ...
            scartLog::logLine("E-ScartImportICCAMV3; exception on line " . $err->getLine() . " in " . $err->getFile() . "; message: " . $err->getMessage());
        }
     ...
     * (this is based on assumption EOKM/SCART also collect when LEA has been informed and when ISP has been inforrmed)
    ...
    private function parseContent($type,$scartimports) {
        ...
        foreach ($scartimports as $reportId => $reportRecord) {
            ...
            /**
             * Note:
             * we got a couple of content items belonging to a ICCAM report
             * In these content items the main content (isSourceUrl) is not always included
             * So we have to loop through the report->reportContents to get the main one
             *
             */
            ...
            foreach ($mainreport->reportContents as $reportContent) {
                $contentReport = (new ScartICCAMapi())->getContent($reportContent->id);
                if ($contentReport->url->isSourceUrl) {
                    // we got 'm
                    $mainContentId = $reportContent->id;
            ...
            if ($mainContentId) {
                if (!scartICCAMinterface::alreadyICCAMreport($reportId,$mainContentId)) {
                    ...
                    scartLog::logLine("D-ScartImportICCAMV3; (mainurl) reportId=$reportId (contentId=$mainContentId) ALREADY imported - skip import");
                }
            } else {
        ...
        if (Input::where('url', $contentItem->url->urlString)->count() == 0) {
            ...
            if ($maininput) {
                ...
                foreach ($reportRecord->contentItems as $contentItem) {
                    if ($contentItem->id != $mainContentId) {
                        // note: strange if here and already imported -> still validate
                        if (!scartICCAMinterface::alreadyICCAMreport($reportId,$contentItem->id)) {
                            if (Input::where('url', $contentItem->url->urlString)->count() == 0) {
                                // get content and analyzed
                                $input = $this->insertContent($reportId,$contentItem->id,$contentItem,$reportRecord,$inputParentId);
                                ...
                                // set reference to SCART
                                scartLog::logLine("D-ScartImportICCAMV3; contentId=$contentItem->id, set ICCAM hotline reference on $input->filenumber");
                                ...
                                scartLog::logLine("W-ScartImportICCAMV3; (content item) url '".$contentItem->url->urlString."' ALREADY imported - skip import");
                            }
                        } else {
                            // only connect
                            scartLog::logLine("D-ScartImportICCAMV3; (content) reportId=$reportId, contentId=$mainContentId ALREADY imported");
                            $input = scartICCAMinterface::findICCAMreport($reportId,$contentItem->id);
                ...
                scartLog::logLine("E-ScartImportICCAMV3; cannot find ICCAM (mainurl) !?!");
            }
            ...
            // To-Do; report back to ICCAM the actions done in SCART
    ...
    private function insertContent($reportId,$contentId,$contentItem,$reportGeneral,$inputParentId) {
        ...
        $this->addLogline("filenumber=$input->filenumber; import url '$input->url', ICCAM contentId=$contentItem->id, received=$input->received_at");
        ...
        if (count($contentItem->assessments) > 0) {
            ...
            if ($classification) {
                ...
                scartLog::logLine("W-ScartImportICCAMV3; assessment given, but no ICCAM classification field found?!? ");
            }
    ...
    private function connect2parent($input,$inputParentId) {
        ...
        if ($inputParentId != $input->id) $this->addLogline("filenumber=$input->filenumber; Connected to parentId: $inputParentId");
        ...
        try {
            ...
            if ($whois['status_success']) {
                ...
                $input->logHistory(SCART_INPUT_HISTORY_HOSTER,
                    $input->host_abusecontact_id,$whois[SCART_HOSTER.'_abusecontact_id'],"Found hoster");
