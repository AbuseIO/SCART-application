...
class scartImportICCAM {
    ...
    public  function do() {
        ...
        try {
            ...
            scartImportICCAM::importJobLastDate($scheduler_process_count);
                                            ...
                                            if (Input::where('url', $item->Url)->count() == 0) {
                                                ...
                                                if ($input) {
                                                    ...
                                                    if (!in_array($input->status_code,[
                                                        ...
                                                        if ($reportID2 = scartICCAMinterface::getICCAMreportID($input->reference)) {
                            ...
                            foreach ($iccamreport->Items AS $item) {
                                ...
                                if (Input::where('url',$item->Url)->count() == 0) {
                                    ...
                                    try {
                                        ...
                                        if ($first) {
                                            $input->note = $note;
                                            $input->reference = scartICCAMinterface::setICCAMreportID($lastID);
                                            $first = false;  // reset
                                        } else {
                                            $input->note = (!empty($item->Memo)) ? $item->Memo : '';
                                            $input->reference = '';
