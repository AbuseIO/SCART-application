...
/**
 * OLD scartMail class for sending email with SMTP transport layer
 * Was needed to get the message-ID; replaced by scartMail with support for Swift (old) and SYmfony (new) message structures
 * Deprecated
 *
 */
...
class scartMailWithSmtp {
    ...
    public static function openSmtpmailer() {

        // Backup default mailer
        self::$_backupsmtp = Mail::getSwiftMailer();

        $setting = self::_getSettings();
        //Log::info("D-openSmtpmailer.settings; host=" . $setting['host'] . ", port=" . $setting['port'] . ", encryption=" . $setting['encryption'] );
    ...
    public static function closeSmtpmailer() {
