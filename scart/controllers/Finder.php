...
class Finder extends scartController
{
        ...
        if (method_exists($this, $method)) {
            try {
                ...
                if (!empty($filters['archiveActive']) && scartArchive::isActiveValid()) {
                    ...
                    $this->vars['archiveActive'] = 1;
                } else {
                    $this->vars['archiveActive'] = 0;
                }

                $bool = $this->$method();

                // don't forget to switch back for other backend general queries
                if (!empty($filters['archiveActive']) && scartArchive::isActiveValid()) {
            ...
            scartLog::logLine("E-Finder(controller): methode not found: ". $method);
        }
    ...
    private function setInputData()
    {
        ...
        // set analist name
        $fieldsClassify['Analist'] = ['span' => 'right', 'label' => 'Analyst', 'type' => 'text', 'readOnly' => true, 'value' =>  $nameAnalist];
